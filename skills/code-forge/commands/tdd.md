...
**When this skill is loaded, you MUST immediately begin executing the Workflow below — do not wait, do not summarize, do not ask "what should I do now". Skills are operational manuals, not reference documents.** read_file Step 0 (Determine Mode), perform it, then enter the appropriate mode (Driven / Auto-Analysis / Standalone) and follow its steps, until the workflow completes or you reach an `ask_user` checkpoint.
...
The first user-visible action of this skill should be either (a) the output of Step 0 / the chosen mode's first step, or (b) an `ask_user` if mode detection needs disambiguation. Never an apology, never a fallback, never silence.
...
This discipline is the upstream defense against patch-first development. read_file it once at the start of every session and again whenever you are tempted to add a new branch / wrapper / parallel module instead of refactoring:
...
|----------|------|----------|
| `@docs/.../test-cases.md` | **Driven Mode** | read_file test cases document, implement each case via TDD |
...
### D.1 read_file and Parse

1. read_file the test-cases document
...
1. **read_file the case** — extract preconditions, steps, expected result, not-expected, test infra
...
3. **RED** — write_file a failing test that matches the case specification
...
5. **GREEN** — write_file minimal production code to make it pass (if the code already exists and passes, the case was already covered — note and move on)
...
- Detect project profile: Web API / CLI Tool / Frontend App / AI generalist / Data Pipeline / Function Library / SDK
...
3. **RED** — write_file a failing test matching the case specification
   - Test name should include TC ID: `test("TC-AUTH-001: create user with valid email returns 201", ...)`
   - Preconditions → test setup; Steps → test actions; Expected result → assertions; Not Expected → negative assertions
4. **VERIFY RED** — Run the test, confirm it fails correctly
5. **GREEN** — write_file minimal production code to make it pass
6. **VERIFY GREEN** — Run all tests, confirm clean pass
7. **REFACTOR** — Clean up if needed
...
#### 1. RED — write_file a Failing Test
...
#### 3. GREEN — write_file Minimal Code
...
|----------------------|-----------|-----|
| write_file production code without a test | STOP — write the failing test first | Tests written after implementation pass immediately and prove nothing |
| Skip testing because the change is "simple" | write_file the test — it will be quick if it's truly simple | Simple code has the sneakiest bugs (off-by-one, null edge cases) |
| Apply a quick fix without a regression test | write_file the test, then fix | Untested fixes become permanent regressions |
...
```
generalist: Add isPalindrome(str) function

1. RED — write_file test:
