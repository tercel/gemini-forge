...
- `./academic-severity-levels.md` — severity rubric (for STILL_STALE / Critical findings during propagation)

**ON-DEMAND (load only at the listed step):**
- `../templates/propagation-report.md` — load at Step 8 (Report Generation)
...
**Report structure**: read the template at `../templates/propagation-report.md` and fill in the `{placeholder}` values, including the Changed Concepts table, per-file downstream updates, and STILL_STALE warnings. Loaded only at this step.
