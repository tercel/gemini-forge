...
1. **Family-based coverage.** Languages are grouped by family (see `./language-data.md`). A document needs ≥3 distinct families per universality claim for adequate coverage.
...
- `./language-data.md` — family-language registry with reference grammars and romanization standards
- `./academic-severity-levels.md` — severity rubric

**ON-DEMAND (load only at the listed step):**
- `./falsifiability-template.md` — load at Step 3a ONLY for Type E (marked-hypothesis) detection on universality claims
- `../templates/cross-lang-audit.md` — load at Step 5 (Report Generation)
...
**Report structure**: read the template at `../templates/cross-lang-audit.md` and fill in the `{placeholder}` values, including the per-claim example-block coverage breakdown and (if --generate or --generate-surface) the draft scaffolds. Loaded only at this step.
