...
5. **Severity discipline.** Apply the rubric in `./academic-severity-levels.md`.

## Reusable Assets

**MANDATORY (load on start):**
- `citation-parser.md` — inline-citation regex set, bibliography schema, canonical key, WebFetch query construction
- `./academic-severity-levels.md` — severity rubric

**ON-DEMAND (load only at the listed step):**
- `./citation-format.md` — load at Step 3 ONLY IF the bibliography style needs detection (e.g., mixed-format suspicion)
- `./replication-and-preregistration.md` — load at Step 5 ONLY IF performing retraction checks on entries with DOIs
- `../templates/cite-audit-report.md` — load at Step 7 (Report Generation)
...
**Report structure**: read the template at `../templates/cite-audit-report.md` and fill in the `{placeholder}` values. Preserve the heading hierarchy and severity-ordering convention. The template is loaded only at this step — not on skill startup — to save token budget for the main detection workflow.
