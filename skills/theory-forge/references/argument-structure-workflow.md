...
- `./argument-patterns.md` — full Toulmin model, fallacy catalog, detection heuristics, output schema
- `./academic-severity-levels.md` — severity rubric

**ON-DEMAND (load only at the listed step):**
- `./falsifiability-template.md` — load at Step 2 ONLY for Type A/B/C/D/E gating (skip Type B/C/D claims from Toulmin checks)
- `../templates/argument-structure-report.md` — load at Step 6 (Report Generation)
...
**Report structure**: read the template at `../templates/argument-structure-report.md` and fill in the `{placeholder}` values, including the Toulmin Completeness table and the Fallacy detections table. Loaded only at this step.
