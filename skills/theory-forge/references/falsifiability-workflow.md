...
- `./falsifiability-template.md` — the four-block schema (claim / methodology / falsification / adjacent evidence) plus claim taxonomy A/B/C/D/E
- `./academic-severity-levels.md` — severity rubric

**ON-DEMAND (load only at the listed step):**
- `../templates/falsifiability-audit.md` — load at Step 7 (Report Generation)
...
**Report structure**: read the template at `../templates/falsifiability-audit.md` and fill in the `{placeholder}` values. The template includes the Type-A-vs-E "choose one fix" template (a/b/c) for unmarked-speculation findings. Loaded only at this step.
