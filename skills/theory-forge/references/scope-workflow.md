...
- `./argument-patterns.md` — load at Step 2 ONLY for the Toulmin Qualifier cross-reference
- `./falsifiability-template.md` — load at Step 3 ONLY for Type A/B/C/D/E gating (Type B/C don't require scope qualifiers)
- `../templates/scope-audit.md` — load at Step 5 (Report Generation)
...
**Report structure**: read the template at `../templates/scope-audit.md` and fill in the `{placeholder}` values. The template includes the Document-Level Findings section that catalogs per-file presence of Limitations / "What X Does NOT Claim" subsections. Loaded only at this step.
