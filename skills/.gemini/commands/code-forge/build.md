...
5. If ambiguous (input is a single word that matches a spec but looks like a prompt), ask user via `ask_user`: "I found `docs/features/{match}.md`. Use this existing spec, or treat your input as a new feature request?"
...
These are used for all file paths in subsequent steps. If unclear, ask the user via `ask_user`: "What should I call this feature? (e.g., 'csv-export')"
...
Ask targeted questions via `ask_user` based on the analysis:
...
`write_file` to `docs/features/{feature-slug}.md`. This becomes the feature spec for subsequent steps.
...
Ask user via `ask_user`: "Review the requirements before proceeding?"
...
- **If exists**: `read_file` it, display summary ("Found existing test-cases.md with {N} cases"), ask user via `ask_user`: "Use existing? Regenerate? Skip test cases?"
...
Ask user via `ask_user`: "Review the test cases before proceeding?"
...
- **If exists and has pending tasks**: Ask user via `ask_user` "Resume existing plan? Regenerate?"
...
Ask user via `ask_user`: "Review the plan before implementing?"
...
- Ask user via `ask_user`: "Fix issues and re-review? Or proceed to verify?"
