...
If you find yourself about to say "the skill didn't produce output", "skill 仍未输出", "falling back to manual port", "回退到手动 port", or anything similar, **STOP**. You have misunderstood how skills work. Go directly to the first executable step and start executing.
...
Spawn a `generalist` sub-agent (`subagent_type: "general-purpose"`):
