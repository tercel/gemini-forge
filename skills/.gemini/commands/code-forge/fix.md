...
**When this skill is loaded, you MUST immediately begin executing the Workflow below — do not wait, do not summarize, do not ask "what should I do now". Skills are operational manuals, not reference documents.** read_file Step 0.1 (Detect Multi-Repo Mode), perform it, then Step 0.5, then Step 1, etc., until the workflow completes or you reach an `ask_user` checkpoint.
...
If you find yourself about to say "the skill didn't produce output", "skill 仍未输出", "falling back to manual fix", "回退到手动修复", or anything similar, **STOP**. You have misunderstood how skills work. Go directly to Step 0.1 of the Detailed Steps and start executing.

The first user-visible action of this skill should be either (a) the output of Step 0.1 / Step 1 of the workflow, or (b) an `ask_user` if a step needs disambiguation. Never an apology, never a fallback, never silence.
...
If `--repos` is present (without `--review`): first, locate the skill installation directory by finding this `fix.md` file's parent. Then dispatch `generalist(subagent_type="general-purpose", description="Fix --repos coordinator")` with prompt containing the **resolved absolute paths**:

> Read these two files and follow them exactly:
> 1. `{resolved_absolute_path}/references/shared/multi-repo.md` — the protocol steps MR-1~MR-5
> 2. `{resolved_absolute_path}/references/fix/multi-repo-defs.md` (if it exists, otherwise use standard fix logic) — the fix-specific definitions
>
...
- **File reference** (e.g., `/code-forge:fix @issues/bug-123.md`) — `read_file` the file for bug details
...
2. **Saved file (fallback):** Only if no review report exists in the current conversation:
   - With feature name (`--review my-feature`): `read_file` `{output_dir}/{feature_name}/review.md`
   - Without feature name (`--review`): `grep_search` `{output_dir}/project-review.md` first, then scan `{output_dir}/*/review.md`
...
2. For each feature, check if the bug-related files overlap with the feature's task files (`read_file` `tasks/*.md` → "Files Involved" sections)
...
Spawn a `generalist` tool call with:
