---
description: "Use when facing 2+ independent problems that can be solved concurrently — dispatches one sub-agent per problem domain for parallel investigation and fixing."
---
# Code Forge — Parallel

## ⚡ Execution Entry Point (READ THIS FIRST)

**When this skill is loaded, you MUST immediately begin executing the Workflow below — do not wait, do not summarize, do not ask "what should I do now". Skills are operational manuals, not reference documents.** Go directly to the first executable step and start executing.
...
If you find yourself about to say "the skill didn't produce output", "skill 仍未输出", "falling back to manual handling", "回退到手动并行处理", or anything similar, **STOP**. You have misunderstood how skills work. Go directly to the first executable step and start executing.

The first user-visible action of this skill should be either (a) the output of the first executable step, or (b) an `ask_user` if a step needs disambiguation. Never an apology, never a fallback, never silence.
...
**CRITICAL:** Launch all sub-agents in a single message using multiple `generalist` tool calls. This enables true parallel execution.

### Step 4: Review and Integrate

After all sub-agents complete:

1. **Check for conflicts** — did any sub-agents modify the same file?
...
```
Parallel dispatch complete: {N} sub-agents, {M} problems resolved

Sub-agent 1: {problem} → {root cause} → {status}
Sub-agent 2: {problem} → {root cause} → {status}
Sub-agent 3: {problem} → {root cause} → {status}
...
Dispatch 3 agents in a single message:
  Sub-agent 1: "Fix auth.test.ts — files: src/auth/, error: missing token validation"
  Sub-agent 2: "Fix payment.test.ts — files: src/payment/, error: decimal precision"
  Sub-agent 3: "Fix email.test.ts — files: src/email/, error: template not found"
