...
If you find yourself about to say "the skill didn't produce output", "skill 仍未输出", "falling back to manual status check", "回退到手动状态检查", or anything similar, **STOP**. You have misunderstood how skills work. Go directly to Step 1 of the Detailed Steps and start executing.
...
2. `grep_search` for all `state.json` files in **both** locations:
...
@../references/shared/overview-generation.md
