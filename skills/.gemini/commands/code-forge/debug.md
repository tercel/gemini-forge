...
If you find yourself about to say "the skill didn't produce output", "skill 仍未输出", "falling back to manual debugging", "回退到手动调试", or anything similar, **STOP**. You have misunderstood how skills work. Go directly to Phase 0 of the Detailed Steps and start executing.

The first user-visible action of this skill should be either (a) the output of Phase 0 / Phase 1 of the workflow, or (b) an `ask_user` if a step needs disambiguation. Never an apology, never a fallback, never silence.
...
2. **Compare against references** — `read_file` reference code COMPLETELY, not skimmed
...
- **>= 3 attempts:** **STOP.** This is NOT a hypothesis failure — it's likely a **wrong architecture** or **wrong mental model.** Discuss with the user via `ask_user` before proceeding.
...
Phase 1 — Investigate:
  $ grep_search "getProfile" src/  →  found in UserService.ts:42, UserController.ts:15
